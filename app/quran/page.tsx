...
const QuranPage = () => {
  return (
    <div className="grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 w-full gap-3">
      {surahData.map((surah) => (
        <Card
          className="h-24 shadow-none hover:shadow-md transition-shadow cursor-pointer bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#fafafa] to-[#fbfbfb]"
          key={surah.number}
        >
          <CardContent className="p-2 flex flex-col justify-center h-full">
            <CardTitle className="mb-4">
              <Button className="rounded-full text-[8px] bg-muted text-primary hover:text-muted h-5 w-5 p-2">
                {surah.number}
              </Button>
            </CardTitle>
